...
class TAG(object):
    ...
    def tag_info(self):
        """Return string with class, name and unnested value."""
        return self.__class__.__name__ + (
            '(%r)' % self.name if self.name
            else "") + ": " + self.valuestr()

    def valuestr(self):
        """Return string of unnested value. For iterators, this
        returns a summary."""
    ...
    def pretty_tree(self, indent=0):
        """Return formated string of self, where iterable items are
        recursively listed in detail."""
        return ("\t" * indent) + self.tag_info()

    def __str__(self):
        """Return a string with the result in human readable format. Unlike valuestr(), the result
    ...
    #  regular iterators
    def __repr__(self):
        """Return a string describing the class, name and id for debugging purposes."""
        return "<%s(%r) at 0x%x>" % (
...
class NBTFile(TAG_Compound):
    ...
    def __repr__(self):
        """
        Return a string describing the class, name and id for
        debugging purposes.
        """
        if self.filename:
